...
// ---------------------------------------------------------------------------
// RDS-backed score store (agent_score namespace). Replaces the old Supabase /
// PostgREST data path (FER-11415). All reads/writes go through lib/db.ts.
// ---------------------------------------------------------------------------
...
function rowToCompany(row: ScoreRow): CompanyScore {
  // pg returns timestamptz columns as Date objects; normalise to an ISO string
  // so downstream display/sort is stable.
  const scoredAt =
...
export async function upsertScore(company: CompanyScore): Promise<void> {
  ...
  // caller explicitly provides them, preserving manual overrides.
  const cols: string[] = [
